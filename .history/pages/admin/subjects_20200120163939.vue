    ...
    >
      <template v-slot:subject>
        <v-row>
          ...
          > </v-divider>
          <v-col md-6 sm-12>
            <v-list dense class="pa-0 ma-0" v-if="subjectDetails.length">
              <v-list-item
  ...
  export default {
    ...
    methods: {
      ...
      clickedAssignTeacher(val) {
        let subId = val.item.id;
        const url = '/subject_details/' + subId
        // for reference inside promise
        let self = this
        this.$axios.get(url)
          .then (function(response) {
      ...
      assignTeacher() {
        ...
        this.$axios.post(url, {subjectId: subject, semester: semester, faculty: faculty, teacherId: 11})
          .then (function(response) {
            alert('Assigned')
            clickedAssignTeacher(subject)
        })
      },
