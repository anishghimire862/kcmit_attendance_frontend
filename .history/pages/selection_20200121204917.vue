  ...
  computed: {
    computedFaculties () {
      ...
      let mappedData = removeDublicates.map(x => {
        let filteredData = this.faculties.filter(y =>  x == y)
        return filteredData
      })
      return mappedData
    },
    computeSemester() {
      return this.computeInfoToDisplay('semester', this.semesters)
    },
    computeFaculty() {
      return this.computeInfoToDisplay('faculty', this.faculties)
    }
  },

  methods: {
    computeInfoToDisplay(fieldName, existingArr) {
      let arrToBePopulate = []
      this.subjects.map(x => {
        if(fieldName === 'semester')
          arrToBePopulate.push(x.semester)
        if(fieldName === 'faculty')
          arrToBePopulate.push(x.faculty)
      })

      let removeDublicates = [...new Set(arrToBePopulate)]

      let mappedData = removeDublicates.map(x => {
        let filteredData = existingArr.filter(y =>  x == y)
        return filteredData
      })
      return mappedData
    },
